...
放在**阶段 2**：进对局时算好的分数要存下来（见下一节），存储层得先有。
...
| | 请求数 | 接口 |
|---|---|---|
| 粗查 | 每人 1 次 | `/api/analysis/player`（6 小时缓存） |
| 龙区分 | 每人再 1 次 | `/api/players/matches?limit=20`（1 小时缓存） |

一局 10 人：10 次 → 20 次。但合并后总量可能反而下降——现在打完再一个个点，
点到后面时前面的人对局列表缓存已过期，等于白拉。开局一次算好、存下来，后面点开都是读存档。

做法：

1. **进对局自动两轮**。第一轮拉 10 个人的玩家分析（约 12 秒，列表先填满 ELO/胜率/偏好兵种），
   第二轮补龙区分（再约 12 秒，分数陆续亮起来）。请求仍然严格串行，间隔按设置。
2. **界面只有一个玩家窗口**，不再有粗查/细查之分：一个窗口里同时显示 ELO、胜率、偏好兵种、
   龙区分、分项百分位、角色构成、最近对局。状态只有「正在算」和「算好了」。
3. **算好就存本地库**（对局 ID + 玩家 ID 一条快照）。重启、翻档案都不用重查，
   还顺手有了「上次遇到 6.2，这次 7.8」这种对比。
4. 新号/没有排位局的人，第二轮那次请求会白发，躲不掉，一局最多一两个。
5. 玩家窗口留一个「重新查询」按钮，强制绕过缓存。
...
3. **版本号重新从 1.0.0 起算**，代价是 4.0.x 的自动更新认不出来（1.0.0 比 4.0.3 小），
   老版用户头一次必须手动装。老仓库 README 和 Release 说明里写清楚下载地址。
   装上之后由新仓库的自动更新接手，以后就是自动的。
4. 新版发布前，4.0.x 继续修 bug，朋友不受影响。
...
| 2 | 主进程骨架：配置、本地库、日志监听、BATrace 客户端、搜索、合并后的玩家查询 | 能开着软件进游戏，看到当前对局里每个人的完整信息（含龙区分），关掉再开还在 |
| 3 | 复盘页、对局档案、上一局 | 复盘页和 4.0.3 显示一致（拿同一局对拍） |
| 4 | 玩家追踪、卡组、封禁（改成启动查一次 + 手动刷新） | 老数据能读出来 |
| 5 | 录像 | 录一局，画质/声音/HDR 都对 |
| 6 | 打包、自动更新、发 1.0.0 | 装上能覆盖旧版且数据不丢；之后的更新走自动 |
