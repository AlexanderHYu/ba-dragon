...
（哪个是哪个：0.5 那个被丢进随机数函数里做判定，0.33 那个进了伤害预估的算式，
所以不会搞反。）弹药表里还有 `RadioFuseDistance`（起爆距离）和
`RADIUFUSE_MISS_DISTANCE_PROPORTION_MIN/MAX`，凑齐就能按溅射衰减曲线算准确值，
但 `RadioFuseDistance` 要重新导出一次数据才有——在那之前计算器用 0.33，
判据退回「制导 + 有溅射 + 打飞行目标」，界面上标了「近炸 ×0.33」。
