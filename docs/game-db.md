...
`ShellHitSystem.DealAOEDamage` 最后是直接调 `DealUnitDamage` 的（`call 0x1807e9450`），
所以**溅射和直击走的是同一条路**：距离衰减系数先乘在基础伤害上，再过穿深/装甲的伤害公式，
步兵抗打击和楼房减伤也照样乘。

这和「先算出直击伤害、再按衰减比例缩」不是一回事——动能弹有 10% 下限、破甲弹是条曲线，
先缩和后缩的结果不一样。之前溅射曲线画的是**裸伤害**，等于默认目标没有装甲：

| | 裸数据 | 过装甲后 |
| --- | --- | --- |
| Kh-55SM 打 T-90M 顶部 | 60 | **50.1**（致死半径 60 m） |
| Kh-38M 打近卫摩步 | 23 | **11.96**（步兵抗打击 ×0.52，炸不死 40 血的班） |

致死半径过了装甲公式之后没有闭式解，改成二分找交点。

### 没有 100 米截断（修正）

早先按一份外部审计写成了 `d = clamp(到外壳的距离, 0, 100)`，**这是错的**。
`DealAOEDamage` 里那个 100 是 `AOE_SEARCH_EXTRA_RADIUS`，用途是把空间查询的半径撑大一圈
（`半径 + 100` 传给八叉树查询，见 `call 0x1802ae230` 前的 `addss xmm0, xmm15`），
不在伤害公式里。衰减本身就三行：

```
d >= 半径              → 0
弹药标了 NoDamageFalloff → 1
否则                    → clamp(1 − d / 半径, 0, 1)
```

影响的是半径大于 100 世界单位的弹药——数据里有 10 种（FAB-9000 175、GBU-57 130、
FAB-3000 110、核弹和匕首 350）。之前它们在 100 之外被算成零伤害。

## 按外部审计复核后改掉的四处

朋友做了一份 21 条的对照审计（他自己的 simulator vs 这个仓库的 `ccd2be4`）。
逐条回到机器码核过之后，确认并改掉了四处：

### 1. 通用装甲值的目标，动能弹也走破甲曲线

伤害派发器 `CalculateHitDamage`（RVA 0x7B3070）的分支顺序是：

```
test edi, edi            ; 装甲类型 0 → 满伤，最前面短路
cmp  edi, 2 / je         ; 类型 2 → 破甲曲线
cmp  dword [armor+0x28], 0 / jg   ; **目标 ArmorValue > 0 → 也走破甲曲线**
                         ; 只有这之后才是动能公式
```

`+0x28` 就是 `Armors.ArmorValue`。**步兵、飞机、轻甲车挨动能弹时全走破甲曲线**，
之前按弹药类型选公式是错的。7.62 打近卫摩步：单发 0.39 → **0.29**（138 发 → 182 发）。

### 2. 穿深公式

`GetPenetration`（RVA 0x7B6DD0）：

```
装甲类型 0  → FLT_MAX
装甲类型 1  → Pmin − (Pmin − Pfar) × 距离 ÷ (参考射程 − 最小射程)，夹到 ≥ 0
其它（破甲）→ 不随距离变，一直是 Pfar
```

三处和之前不一样：**分母要减最小射程**、**破甲弹不衰减**、**结果不取整**。
参考射程是 `GetDispersionReferenceRange`：地面 / 低空 / 高空里第一个正数。

### 3. 装填不叠加瞄准

`ShotCycleAndReload` 给连发和装填各挂**独立的计时器**，盯着同一个目标时不会重新瞄准。
之前写的是「装填 + 瞄准」，现在是 `max(装填, 连发间隔)`；瞄准只在第一次开火前算一次。

### 4. 同通道抢占看优先级不看伤害

`CanUseFiringChannel` 里比的是 `WeaponPriority`，**数字小的赢**（0x7B4753/767），
不是秒伤。这个字段本来没提取，现在进 `turretWeapons` 了（数据里 0 占 1029 条、
1 占 171、2 占 69、3 占 10）。平手时它还会比 `TargetComponent+8` 的一个分数——
那个算不出来，平手就退回按秒伤挑。

### 审计里判我们错、但复核之后不成立的

**BA-02（班组同名武器按人数乘）**。`CalculateWeaponTimeTillNextShot` 的尾巴是
`xmm0 /= UsingAliveSoldierCount`——**射速是除以活着的操作手数的**，N 个人拿同一把枪
不是同时打，而是间隔变成 1/N。这和我们「一次 N 倍伤害、节奏不变」在总秒伤上等价。
