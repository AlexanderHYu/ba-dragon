...
游戏自带一张加密的单位表（单位、配装、价格）。在「设置 → 游戏数据」里填上密钥之后，
复盘里的配装会显示真名（「配装 A」→「M1A1 FEP Trophy」）、花费从估算变成精确值，
卡组工具也能直接看卡组内容。**密钥不随本仓库发布**，不填不影响其它功能。
怎么读的、准不准，见 [docs/game-db.md](docs/game-db.md)。
