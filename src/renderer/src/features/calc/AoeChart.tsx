...
export default function AoeChart({
  ...
  const x = (d: number): number => pad.l + (d / radius) * (W - pad.l - pad.r)
  ...
  const area = path + ' L' + x(radius) + ',' + y(0) + ' L' + x(0) + ',' + y(0) + ' Z'
  const ticks = [0, 0.25, 0.5, 0.75, 1].map((f) => Math.round(radius * f))
  ...
  return (
    <svg className="aoe-chart" viewBox={'0 0 ' + W + ' ' + H} width="100%" height={H}>
      ...
      {overpressure > 0 && (
        <rect x={pad.l} y={pad.t} width={Math.max(0, x(overpressure) - pad.l)} height={H - pad.t - pad.b} fill="var(--bad)" opacity="0.12" />
      )}
